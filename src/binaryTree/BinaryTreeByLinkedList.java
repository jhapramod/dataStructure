...
public class BinaryTreeByLinkedList {
	...
	BinaryNode mirrorOfBinaryTree(BinaryNode node) {
		if(node!=null) {
			mirrorOfBinaryTree(node.getLeft());
			mirrorOfBinaryTree(node.getRight());
			
			BinaryNode tempNode =node.getLeft();
	...
	int heightOfBinaryTree(BinaryNode node) {
		int height=0;
		if(node==null)
			return 0;
	int lefHeight=	heightOfBinaryTree(node.getLeft());
